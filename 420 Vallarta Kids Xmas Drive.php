<?php
include 'header.php';
?><title>420 Puerto Vallarta Xmas Kids Toy Drive - Cannabis Puerto Vallarta</title>
<div class="container"><div class="row"><div class="col-md-12">
  <h1>420 Vallarta Christmas Kids Drive<br />
    <br />
    <img src="images/420 Vallarta Kids Christmas Toy Drive.png" alt="420 Vallarta Christmas Toy Drive" width="100%" height="100%" longdesc="https://420cdmx.co/420 Vallarta kids Xmas Drive.html" /><br />
</h1>
            <p>We are beginning our Christmas drive to help some of the local communities.</p>
            <p>We are accepting donations of clothes (New or Used)<br />
              Any Cash Donations, We will Match</p>
            <p>The money and items will be donated to local kids in impoverished communities.<br />
              <br />
      </p>
            <h2>Used Items </h2>
            <p>Shirts, blouses, jeans, pants, dresses, shorts, shoes  </p>
            <h2>New Items </h2>
            <p>Socks, underwear<br />
              A fresh packet of socks or underwear from Costco or Walmart will be of great help.<br />
            </p>
            <h2>Cash Donations  </h2>
            <p>We will match all cash donations.<br />
              Please Fund us at GoGetFunding <br />
                <a href="https://gogetfunding.com/christmas-kids-drive-puerto-vallarta/"> Donate Here</a>
<br />
                420 Vallarta Appreciates Your Help
              <br />
                <br />
      </p>
            <p>Support us on <br />
                <a href="https://www.facebook.com/420cdmxco" target="_blank">Facebook</a> <br />
                <a href="https://x.com/420cdmxco" target="_blank">Twitter</a> <br />
                <a href="https://www.instagram.com/420cdmxco/" target="_blank">Instagram</a><a href="https://www.instagram.com/420cdmxco/" target="_blank"></a> <br />
                <a href="https://www.youtube.com/@420.vallarta" target="_blank">YouTube<br />
                </a><a href="https://www.pinterest.com.mx/420cdmxco/" target="_blank">Pinterest</a><a href="https://www.youtube.com/watch?v=7hHK9H6CgQ0" target="_blank"><br />
                </a><br />
                <br />
              You can also reach us by email
              Info@420cdmx.co </p>
            <p><a href="https://420cdmx.co/">420vallarta.com</a></p>
            <h2>&nbsp;</h2>
      </div>
    </div>
</div>
<?php
include 'footer.php';
?>
